...
namespace SofttekFinalProjectBackend.Services
{
    public class AccountsServices
    {
        ...
        public async Task<IActionResult> GetAccount(string cbu, int parameter, int pageSize, int pageToShow, HttpRequest Request)
        {
            try
            {
                ...
                if (parameter == 0)
                {
                    FiduciaryAccount account = await _unitOfWork.FiduciaryAccountRepository.GetAccount(cbu,"","");
